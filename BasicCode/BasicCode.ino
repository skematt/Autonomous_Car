...
short fuse_sensors() {
  ...
  short fused = ((-8)*result[0] - 4*result[1] - 2*result[2] - result[3] + result[4] + 2*result[5] + 4*result[6] + 8*result[7]) / 4;
  return fused;
...
void loop() {
  ...
  const unsigned char BASE_SPD = 50;

  
  
  short sensor_fusion = fuse_sensors();
  
  // Serial.println(sensor_fusion);
  // delay(500);
  
  float p = sensor_fusion * K_p;


  // TODO: Change this to test different speed functions
  analogWrite(L_PWM,BASE_SPD - p);
  analogWrite(R_PWM,BASE_SPD + p);
  ...
  // Serial.println(DONUT_CONDITION);
  // delay(200);
